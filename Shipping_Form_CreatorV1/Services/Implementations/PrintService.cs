...
public class PrintService
{
    public List<UserControl> BuildAllPackingListPages(MainViewModel viewModel)
    {
        ...
        if (report.LineItems.Count > 1)
        {
            ...
            var pageNum = 2;
            for (var i = 1; i < report.LineItems.Count; i += itemsPerPage)
            {
                var items = new ObservableCollection<LineItem>(
                    report.LineItems.Skip(i).Take(itemsPerPage)
                );
    ...
    public void PrintPackingListPages(List<UserControl>? pages)
{
    ...
    foreach (var printArea in pages.Select(page => page.FindName("PackingListPage1PrintArea") as FrameworkElement ??
                                                   page.FindName("PackingListPage2PlusPrintArea") as FrameworkElement ??
                                                   page.FindName("PackingListNotesPage1PrintArea") as FrameworkElement).OfType<FrameworkElement>())
    {
